#added 15 flag to show command 14 active
...
command14_active = False
command11_start_time = None
command11_active = False
integral_reset_time = 20 #seconds

yaw_averaging_active = False
yaw_deg_list = []
...
while True:
    ...
    if marker_IDs is not None:
        ...
        if marker_corners:
            ...
            command = ""
            if round(tVec[1][0], 1) > 0.1: #threshold changed from 0.05 and added rounding to reduce sensitivity
                command += "3%"
            elif round(tVec[1][0], 1) < -0.1: #threshold changed from -0.05 and added rounding to reduce sensitivity
                command += "2%"

            if tVec_distance_combo > 0.05 and yaw_deg > 4:
                command += "7?" + str(round(yaw_deg)) + "%"

            elif tVec_distance_combo < -0.05 and yaw_deg < -4:
            ...
            elif tVec_distance_combo <= 0.05 and tVec_distance_combo >= -0.05 and not yaw_averaging_active and not command11_active:
                command += "12%"

            if distance > 40:
                command += "10?" + str(round(distance)) + "%"
                
            if distance <= 40 and not command11_active: # removed "and not yaw_averaging_active" condition
                command += "13%"
                
            if distance <= 40 and tVec_distance_combo <= 0.05 and tVec_distance_combo >= -0.05 and yaw_deg <= 4 and yaw_deg >= -4:
                yaw_averaging_active = True
    ...
    if (current_time - last_command_time) >= COMMAND_INTERVAL:
        if command == "11%":
            if not command11_active and not command14_active:
                #First time sending command 11
                command11_active = True
                command11_start_time = current_time
            elif not command14_active:
                elapsed = current_time - command11_start_time
                if elapsed >= integral_reset_time:
                    print("Resetting integral")
                    command += "14%"
                    command11_active = False
                    command11_start_time = None
                    command14_active = True
                    
        elif command != "12%13%":
            #Other command interrupting stream of command 11
            command11_active = False
            command11_start_time = None
            command14_active = False
